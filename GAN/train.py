...
optimizer_D = Adam(model.D.parameters(),lr = 2e-4, momentum= .5)
optimizer_G = Adam(model.G.parameters(),lr = 2e-4, momentum= .5)

pbar = tqdm(range(n_epoch))
loss_plot = {}
fixed_sample= torch.rand(10,100)
for ep in pbar:
    ...
    if ep % save_result == 0:
        gen = model.generate(fixed_sample).view(-1, 1, 28, 28)
